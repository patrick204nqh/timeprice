...
module Tools
  module DataPipeline
    ...
    # written via Tools::DataPipeline::FxAnnualFile.
    class FxYearFile
      def initialize(year)
        @year = year.to_i
      end
      ...
      def path
        File.join(Tools::DataPipeline::DATA_ROOT, "fx", "usd", "#{year}.json")
      end

      def load_prior
        Tools::DataPipeline.read_json_if_exists(path) || {}
      end
    end
    ...
    class FxAnnualFile
      ...
      def self.path
        File.join(Tools::DataPipeline::DATA_ROOT, PATH_REL)
      end
      ...
      def self.write(annual_by_year_currency:, provider_id:, source_label:)
        prior = Tools::DataPipeline.read_json_if_exists(path) || {}
